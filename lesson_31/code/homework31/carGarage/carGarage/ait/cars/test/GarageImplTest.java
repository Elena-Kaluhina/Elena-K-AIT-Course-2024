package homework31.carGarage.carGarage.ait.cars.test;

import homework31.carGarage.carGarage.ait.cars.dao.Garage;
import homework31.carGarage.carGarage.ait.cars.dao.GarageImpl;
import homework31.carGarage.carGarage.ait.cars.model.Car;
...
class GarageImplTest {
    ...
    // Тесты сортировки, интегрированные с остальными

    @Test
    void sortByBrandTest() {
        Car[] sortedByBrand = garage.sortByBrand();
        Car[] expected = {cars[0], cars[2], cars[1], cars[3]}; // порядок по модели
        assertArrayEquals(expected, sortedByBrand);
    }

    @Test
    void sortByColorTest() {
        Car[] sortedByColor = garage.sortByColor();
        Car[] expected = {cars[1], cars[3], cars[0], cars[2]}; // порядок по цвету
        System.out.println("Expected: " + Arrays.toString(expected));
        System.out.println("Sorted: " + Arrays.toString(sortedByColor));
        assertArrayEquals(expected, sortedByColor);
    }

    @Test
    void sortByCompanyTest() {
        Car[] sortedByCompany = garage.sortByCompany();
        Car[] expected = {cars[0], cars[1], cars[2], cars[3]}; // порядок по компании
        assertArrayEquals(expected, sortedByCompany);
    }
}
